...
use codex_app_server_protocol::ThreadForkActiveMode;
...
fn rollout_path(codex_home: &Path, timestamp: &str, conversation_id: &str) -> PathBuf {
    codex_home
        .join("sessions")
        .join("2025")
        .join("01")
...
async fn append_sampling_boundary(path: &Path) -> Result<()> {
...
fn thread_contains_agent_text(thread: &Thread, needle: &str) -> bool {
    thread.turns.iter().any(|turn| {
        turn.items.iter().any(
            |item| matches!(item, ThreadItem::AgentMessage { text, .. } if text.contains(needle)),
        )
    })
}
...
#[tokio::test]
async fn thread_fork_non_interrupting_excludes_partial_output() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let codex_home = TempDir::new()?;
    create_config_toml(codex_home.path(), &server.uri())?;
    ...
    assert_eq!(fork_result.get("appliedSnapshotMode"), None);
    assert_eq!(fork_result.get("snapshotFallbackReason"), None);
    assert_eq!(thread.forked_from_id, Some(conversation_id));
    assert_eq!(thread.preview, preview);
    assert!(
        !thread_contains_agent_text(&thread, "partial assistant output"),
        "non-interrupting fork must exclude assistant output after the sampling boundary"
    );
...
#[tokio::test]
async fn thread_fork_non_interrupting_without_boundary_fails_closed() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let codex_home = TempDir::new()?;
    create_config_toml(codex_home.path(), &server.uri())?;
    ...
    assert!(
        fork_err
            .error
            .message
            .contains("no stable sampling boundary"),
...
#[tokio::test]
async fn thread_fork_ephemeral_non_interrupting_uses_selected_history() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let codex_home = TempDir::new()?;
    create_config_toml(codex_home.path(), &server.uri())?;
    ...
    assert_eq!(thread.preview, preview);
    assert!(
        !thread_contains_agent_text(&thread, "ephemeral partial output"),
        "ephemeral fork response must use selected history, not raw source history"
    );
